...
def captcha():
    ...
    with Stealth().use_sync(sync_playwright()) as p:
        ...
        try:
            ...
            combined_result = page.inner_text('#result')
...
def create_account(session, lock):
    while True:
        prx = None
        with lock:
            if PROXY:
        ...
        try:
            captcha_res = captcha()
            if not captcha_res:
                print(f"НЕУДАЧА [{prx}]: Капча вернула пустоту.")
                continue
            ticket, randomstr = captcha_res.split(":")
        except Exception as e:
            print(f"НЕУДАЧА [{prx}]: Ошибка парсинга капчи: {e}")
            continue

        # --- ПОДПИСЬ (Строго по алфавиту: q, randomstr, ticket) ---
        # SignInterceptor.java использует TreeMap (сортировка ключей)
        # URLDecoder.decode в java коде означает, что для подписи берутся СЫРЫЕ данные
        params_for_sign = f"q={q}&randomstr={randomstr}&ticket={ticket}"
        
        x_sign = get_x_sign(x_urlpath, x_nonce, x_time, params_for_sign, bmg_id)

        headers = {
            "bmg-user-id": "0", "bmg-device-id": bmg_id, "bmg-sign": bmg_sign,
            "bmg-adid-sign": "b915e18f539d8830fb0bebba670ed63b9cbd1053", "package-name": "com.sandboxol.blockymods.official",
            "userId": "0", "packageName": "official", "packageNameFull": "com.sandboxol.blockymods.official",
            "androidVersion": "30", "OS": "android", "appType": "android", "appLanguage": "en",
            "appVersion": "5421", "appVersionName": "2.125.1", "channel": "sandbox", "uid_register_ts": "0",
            "device_register_ts": "0", "eventType": "app", "userDeviceId": bmg_id, "userLanguage": "en_US",
            "region": "", "clientType": "client", "env": "prd", "package_name_en": "com.sandboxol.blockymods.official",
            "md5": "c0c2f5baf2e9b4a063fc0cdf099960de", "adid": "aae9dcee-9754-4e82-9378-e36618159408",
            "telecomOper": "unknown", "manufacturer": "Redmi_Redmi Note 8 Pro", "network": "wifi", "brand": "Redmi",
            "model": "Redmi Note 8 Pro", "device": "begonia", "deviceModel": "Redmi Note 8 Pro",
            "board": "begonia", "cpu": "CPU architecture: 8", "cpuFrequency": "2012500", "dpi": "2.75",
            "screenHeight": "2220", "screenWidth": "1080", "ram_memory": "5635", "rom_memory": "52438",
            "open_id": "", "open_id_type": "0", "client_ip": "", "apps_flyer_gaid": "aae9dcee-9754-4e82-9378-e36618159408",
            "X-ApiKey": "6aDtpIdzQdgGwrpP6HzuPA", "X-Nonce": x_nonce, "X-Time": x_time, "X-Sign": x_sign,
            "X-UrlPath": x_urlpath, "Access-Token": "", "Connection": "Keep-Alive",
            "Accept-Encoding": "gzip", "User-Agent": "okhttp/4.11.0"
        }

        # --- ОТПРАВКА (В порядке, который нужен: ticket, randomstr, q) ---
        params_to_send = {
            "ticket": ticket,
            "randomstr": randomstr,
            "q": q
        ...
        try:
            ...
            if response.status_code == 200 and response.json().get("code") == 1:
                print(f"УСПЕХ: {bmg_id}:{bmg_sign}")
                with lock:
                    with open("bmg_ids.txt", "a") as f:
                        f.write(f"{bmg_id}:{bmg_sign}\n")
            else:
                print(f"НЕУДАЧА [{prx}]: Статус {response.status_code}, Ответ: {response.text[:100]}")
        except Exception as e:
            print(f"ОШИБКА [{prx}]: {e}")
...
def main():
    ...
    print("Работа завершена. Все прокси отработаны.")
